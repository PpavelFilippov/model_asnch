"""
МОДИФИЦИРОВАННЫЙ СЦЕНАРИЙ: ВСГ раскачивает куст СГ
===================================================
Полный самостоятельный код (без внешних импортов модели).

Цель: показать, что ВСГ с агрессивными настройками провоцирует
межмашинные колебания в кусте из 3 СГ — машины колеблются
в противофазе, не могут поделить мощность и установиться.

Что изменено относительно базовой модели:
1. РАЗНЫЕ параметры СГ (H, Xd, droop, X_line) — асимметрия
2. НИЗКОЕ демпфирование СГ (D_mech=0.2–0.3) — колебания не затухают
3. АГРЕССИВНЫЙ ВСГ (H_virt=0.3, D_virt=2, K_rocof=2, K_ff=1.5)
4. БОЛЬШОЙ виртуальный импеданс Xv=0.35 — сильная связь dP/dt -> dV
5. РАЗНАЯ длина линий СГ — разная электрическая удалённость
6. Медленный губернатор (T_gov=1.5) при быстром ВСГ
7. Серия набросов/сбросов нагрузки — не дать системе установиться
8. Ограничение виртуальной частоты ВСГ (±2 Гц) — физичность
"""
...
def plot_oscillation_showcase(res, n_sg, sg_params_list, vsg_p, save_path=None):
    """
    Главный график: межмашинные колебания в противофазе.
    4 панели: P каждого СГ+ВСГ, частота, dP отклонения, напряжение.
    """
    _setup_sci_style()
    fig, axes = plt.subplots(4, 1, figsize=(10, 14), sharex=True)
    fig.suptitle(
        'ВСГ раскачивает куст СГ: межмашинные колебания в противофазе\n'
        f'3×СГ (H={sg_params_list[0].H}, D={sg_params_list[0].D_mech}) + '
        f'ВСГ (H={vsg_p.H_virt}, D={vsg_p.D_virt}, '
        f'K_rocof={vsg_p.K_rocof}, K_ff={vsg_p.K_ff})',
        fontsize=12, fontweight='bold', y=0.98
    )

    t = res['t']
    sg_labels = [f'СГ-{i+1}' for i in range(n_sg)]

    # (a) Активная мощность каждой машины
    ax = axes[0]
    for i in range(n_sg):
        ax.plot(t, res['P_sg_each'][i] / 1000, color=_SG_COLORS[i],
                lw=1.5, label=sg_labels[i])
    ...
    ax.set_title('(a) Активная мощность каждой машины — противофазные колебания',
    ...
    ax.set_title('(b) Частота шины — ВСГ навязывает колебания', loc='left', fontsize=10)

    # (c) Отклонения dP — чистая противофаза
    ax = axes[2]
    ...
    ax.legend(loc='upper right', fontsize=8)
    ax.set_title('(c) Отклонения P от среднего — видна противофаза колебаний',
                 loc='left', fontsize=10)
    ...
    ax.set_ylabel('V, о.е.')
    ax.legend(loc='upper right', fontsize=8)
    ax.set_title('(d) Напряжение шины — колебания от перетоков P и Q',
                 loc='left', fontsize=10)
    ax.set_xlim(t[0], t[-1])
...
def plot_oscillation_zoom(res, n_sg, sg_params_list, vsg_p,
                          event_time=5.0, window=12.0, save_path=None):
    """Zoom на переходный процесс — детальный вид колебаний."""
    _setup_sci_style()
    t = res['t']
    ...
    fig.suptitle(
        f'ZOOM: Раскачка куста СГ после наброса нагрузки при t={event_time}с\n'
        f'ВСГ мгновенно компенсирует, но раскачивает межмашинные колебания',
        fontsize=12, fontweight='bold'
    )

    # (a) P каждого СГ (Pe и Pm)
    ax = axes[0, 0]
    for i in range(n_sg):
    ...
    ax.set_ylabel('P, МВт')
    ax.set_title('(a) Pe и Pm каждого СГ', loc='left', fontsize=10)
    ...
    ax.set_title('(b) Мощность ВСГ — агрессивный отклик', loc='left', fontsize=10)
    ...
    ax.axvline(event_time, color='red', lw=0.8, ls='--', alpha=0.5)
    ax.set_ylabel('f, Гц')
    ax.set_title('(c) Частота — ВСГ опережает/запаздывает', loc='left', fontsize=10)
    ...
    ax.axvline(event_time, color='red', lw=0.8, ls='--', alpha=0.5)
    ax.set_ylabel('V, о.е.')
    ax.set_title('(d) Напряжение шины и клемм СГ', loc='left', fontsize=10)
    ...
    ax.set_title('(e) АРВ: напряжение возбуждения', loc='left', fontsize=10)
    ax.legend(fontsize=8)
    ...
    ax.set_xlabel('Время, с')
    ax.set_ylabel('ΔP, МВт')
    ax.set_title('(f) Дисбаланс мощности на шине', loc='left', fontsize=10)
...
def plot_antiphase_detail(res, n_sg, sg_params_list, vsg_p,
                          event_time=5.0, window=12.0, save_path=None):
    """Детальный график противофазных колебаний и перекачки мощности."""
    _setup_sci_style()
    t = res['t']
    ...
    fig.suptitle(
        'Перекачка мощности между генераторами\n'
        'СГ не могут поделить нагрузку — ВСГ усиливает колебания',
        fontsize=12, fontweight='bold'
    )
    ...
    ax.set_title('(a) Мощности машин — не могут установиться', loc='left', fontsize=10)

    # (b) Разность P между парами СГ
    ax = axes[1]
    ...
    ax.set_ylabel('ΔP, МВт')
    ax.legend(fontsize=8)
    ax.set_title('(b) Разность мощностей между парами СГ — перетоки',
                 loc='left', fontsize=10)

    # (c) P ВСГ vs суммарный СГ
    ax = axes[2]
    ...
    ax.set_ylabel('P, МВт')
    ax.legend(fontsize=8)
    ax.set_title('(c) ВСГ перехватывает мощность — СГ не успевают',
                 loc='left', fontsize=10)
...
if __name__ == "__main__":
    ...
    print(" Наброс -> ВСГ подхватывает -> раскачка при дележке")
    print("=" * 70)
    ...
    print(f"\nПараметры СГ (одинаковые, «испорченные»):")
    print(f"  H={sg_common.H}с, D_mech={sg_common.D_mech} (было 8.0), "
          f"T_gov={sg_common.T_gov}с (было 0.6), ramp_up={sg_common.ramp_up} (было 0.30)")
    ...
    plot_oscillation_showcase(
        results, N_SG, sg_list_params, vsg,
        save_path=os.path.join(output_dir, "01_oscillation_showcase.png")
    )

    plot_oscillation_zoom(
        results, N_SG, sg_list_params, vsg,
        event_time=5.0, window=18.0,
        save_path=os.path.join(output_dir, "02_oscillation_zoom.png")
    )

    plot_antiphase_detail(
        results, N_SG, sg_list_params, vsg,
        event_time=5.0, window=18.0,
        save_path=os.path.join(output_dir, "03_antiphase_detail.png")
    )
